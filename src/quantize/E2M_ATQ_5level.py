"""5-level E2M-ATQ generalization used for the Qwen routed-expert sweep.
...
This experimental copy keeps TWLA's two-stage row-wise optimization and
manifold relocation, but replaces the ternary codebook by four uniformly
spaced centered levels.  The original E2M_ATQ.py remains untouched.
"""
...
def support_aware_ls(Wc: torch.Tensor, T: torch.Tensor, eps: float = 1e-8) -> torch.Tensor:
    """Least-squares row scale for an arbitrary centered scalar codebook."""
    num = torch.sum(T * Wc, dim=1)
    den = torch.sum(T.square(), dim=1) + eps
    return num / den


@torch.no_grad()
def multilevel_codes(Wc: torch.Tensor, alpha: torch.Tensor) -> torch.Tensor:
    """Assign each centered weight to its nearest one of NUM_LEVELS codes.

    Odd level counts contain zero (for example, five levels are -2..2).
    Even counts use half-integers (four levels are -1.5,-0.5,0.5,1.5).
    Because alpha is fitted independently per row, multiplying all codes by
    a common constant would produce the same reconstruction.
    """
    center = (NUM_LEVELS - 1) / 2.0
    safe_alpha = alpha.abs().clamp_min(1e-8)
    indices = torch.round(Wc / safe_alpha[:, None] + center)
    return indices.clamp_(0, NUM_LEVELS - 1).sub_(center)
...
def euclidean_warm_start(
    ...
    """Stabilize a NUM_LEVELS scalar pattern and Euclidean row parameters.
    ...
    mu = W.mean(dim=1)
    Wc = W - mu[:, None]
    max_code = (NUM_LEVELS - 1) / 2.0
    alpha = Wc.abs().amax(dim=1).div(max_code).clamp_min(1e-8)
    T = multilevel_codes(Wc, alpha)
    ...
    for _ in range(int(iters)):
        ...
        T = multilevel_codes(Wc, alpha)                # nearest-code update
